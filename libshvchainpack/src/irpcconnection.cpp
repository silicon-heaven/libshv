...
int IRpcConnection::callShvMethod(int rq_id, const std::string &shv_path, const std::string& method, const RpcValue &params, const RpcValue &user_id, const shv::chainpack::RpcValue::Map &meta_values)
{
	RpcRequest rq;
	...
	if(!shv_path.empty())
		rq.setShvPath(shv_path);
	for(const auto &[key, value] : meta_values) {
